...
from models.PCA.gconv_pdl import Siamese_ChannelIndependentConv #, Siamese_GconvEdgeDPP, Siamese_GconvEdgeOri
    ...
    def __init__(self):
        ...
        for i in range(self.gnn_layer):
            ...
            if i == self.gnn_layer - 2:  # only second last layer will have cross-graph module
                ...
                self.cross_edge_layer = (nn.Linear(cfg.CIE.GNN_FEAT * 2, cfg.CIE.GNN_FEAT))
    ...
    def forward(self, src, tgt, P_src, P_tgt, G_src, G_tgt, H_src, H_tgt, ns_src, ns_tgt, K_G, K_H, type='img'):
        ...
        for i in range(self.gnn_layer):
            ...
            if i == self.gnn_layer - 2:
                ...
                '''
                cross_graph_edge = self.cross_edge_layer
                emb_edge1 = emb_edge1.permute(0, 3, 1, 2)
                emb_edge2 = emb_edge2.permute(0, 3, 1, 2)
                s = s.unsqueeze(1)
                new_emb_edge1 = cross_graph_edge(paddle.concat((emb_edge1, paddle.matmul(paddle.matmul(s, emb_edge2), s.transpose((0, 1, 3, 2)))), axis=1).permute(0, 2, 3, 1))
                new_emb_edge2 = cross_graph_edge(paddle.concat((emb_edge2, paddle.matmul(paddle.matmul(s.transpose((0, 1, 3, 2)), emb_edge1), s)), axis=1).permute(0, 2, 3, 1))
                emb_edge1 = new_emb_edge1
                emb_edge2 = new_emb_edge2
                '''

        d = hungarian(ss[-1], ns_src, ns_tgt)
        return ss[-1], d
